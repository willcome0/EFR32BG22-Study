...
static const uint8_t led_service[2] = { 0x15, 0x18 };
static const uint8_t thermo_service[2] = { 0x09, 0x18 };
// Temperature Measurement characteristic UUID defined by Bluetooth SIG
static const uint8_t led_char[2] = { 0x56, 0x2a };
static const uint8_t thermo_char[2] = { 0x1c, 0x2a };
// Print out tx power value
static bool print_tx_power = PRINT_TX_POWER_DEFAULT;

static void init_properties(void);
static uint8_t find_service_in_advertisement(uint8_t *data, uint8_t len, const uint8_t* s_uuid);
...
void sl_bt_on_event(sl_bt_msg_t* evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_connection_opened_id:
      ...
      sl_app_assert(sc == SL_STATUS_OK,
                    "[E: 0x%04x] Failed to start power reporting\n",
                    (int)sc);
    ...
    case sl_bt_evt_gatt_service_id: // 保存服务的句柄
      table_index = find_index_by_connection_handle(evt->data.evt_gatt_service.connection);
      if (table_index != TABLE_INDEX_INVALID) {
        uint8array *uuid = &(evt->data.evt_gatt_service.uuid);
         if (memcmp(uuid->data, led_service, uuid->len) == 0) {
           sl_app_log("LED service is discovered\n");
           conn_properties[table_index].led_service_handle = evt->data.evt_gatt_service.service;
         }
         else if (memcmp(uuid->data, thermo_service, uuid->len) == 0) {
           sl_app_log("Thermometer service is discovered\n");
           conn_properties[table_index].thermometer_service_handle = evt->data.evt_gatt_service.service;
           sl_bt_gatt_discover_primary_services_by_uuid(evt->data.evt_connection_opened.connection,
                                                                   sizeof(led_service),
    ...
    case sl_bt_evt_gatt_characteristic_id: // 保存特征值的句柄
      ...
      if (table_index != TABLE_INDEX_INVALID) {
        ...
        if (memcmp(uuid->data, led_char, uuid->len) == 0) {
          sl_app_log("LED characteristic is discovered\n");
          conn_properties[table_index].led_characteristic_handle = evt->data.evt_gatt_characteristic.characteristic;
        }
        else if (memcmp(uuid->data, thermo_char, uuid->len) == 0) {
          sl_app_log("Thermometer characteristic is discovered\n");
    ...
    case sl_bt_evt_gatt_procedure_completed_id:
      ...
      if (conn_state == discover_services 
          && conn_properties[table_index].led_service_handle != SERVICE_HANDLE_INVALID
          && conn_properties[table_index].thermometer_service_handle != SERVICE_HANDLE_INVALID) {
        // Discover thermometer characteristic on the slave device
        if (conn_properties[table_index].led_characteristic_handle == CHARACTERISTIC_HANDLE_INVALID) {
            sl_bt_gatt_discover_characteristics(evt->data.evt_gatt_procedure_completed.connection,
                                               conn_properties[table_index].led_service_handle);
        }
        else if (conn_properties[table_index].thermometer_characteristic_handle == CHARACTERISTIC_HANDLE_INVALID) {
            sl_bt_gatt_discover_characteristics(evt->data.evt_gatt_procedure_completed.connection,
      ...
      if (conn_state == discover_characteristics
          && conn_properties[table_index].led_characteristic_handle != CHARACTERISTIC_HANDLE_INVALID
          && conn_properties[table_index].thermometer_characteristic_handle != CHARACTERISTIC_HANDLE_INVALID) {
        // stop discovering
        sl_bt_scanner_stop();
        sl_app_log("Stop discover characteristic\n");
        // enable indications
...
void print_values(void)
{
  ...
  for (i = 0u; i < SL_BT_CONFIG_MAX_CONNECTIONS; i++) {
    if ((TEMP_INVALID != conn_properties[i].temperature) && (RSSI_INVALID != conn_properties[i].rssi) ) {
      sl_app_log("server_address: %04x\n", conn_properties[i].server_address);
      sl_app_log("   temperature: %2.1f\n", conn_properties[i].temperature);
      uint8_t value = 0;
      if (conn_properties[i].temperature > 26) {
          sl_app_log("Control LED On!\n");
          value = 1;
      }
      sl_bt_gatt_write_characteristic_value(1, conn_properties[0].led_characteristic_handle, 1, &value);
      sl_app_log("\n");
    }
  }
}
