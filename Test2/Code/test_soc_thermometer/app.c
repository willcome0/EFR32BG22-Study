...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    ///////////////////////////////////////////////////////////////////////////
    case sl_bt_evt_gatt_server_user_read_request_id: { // 读请求事件
      sl_bt_evt_gatt_server_characteristic_status_t *status = &evt->data.evt_gatt_server_characteristic_status;
      if (gattdb_led == status->characteristic) {
          uint8_t value = sl_led_get_state(&sl_led_led0);
          sl_bt_gatt_server_send_user_read_response(1, status->characteristic, 0, 1, &value, NULL);
      }
      break;
    }

    case sl_bt_evt_gatt_server_user_write_request_id: { // 写请求事件
